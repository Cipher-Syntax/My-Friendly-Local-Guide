...
import { LayoutDashboard, BookOpen, UsersRound, User, LogOut, Star, Sun, Moon, Wallet, Settings, Map, Home, MessageSquare, History, BarChart3, Compass } from 'lucide-react';
...
export default function AgencySidebar({ activeTab, setActiveTab, handleSignOut, unreadMessages = 0 }) {
    ...
    const menuItems = [
        ...
        { id: 'explore', icon: Compass, label: 'Explore Directory' },
    ...
    return (
        <aside className="w-72 min-w-72 shrink-0 bg-white/90 dark:bg-slate-800/50 backdrop-blur-sm border-r border-slate-200 dark:border-slate-700/50 flex flex-col h-screen transition-colors duration-300 scroll-smooth">

            {/* TOP - Fixed */}
            <div className="shrink-0 p-6 border-b border-slate-200 dark:border-slate-700/50 flex flex-col justify-center">
            ...
            {/* BOTTOM - Fixed */}
            <div className="shrink-0 p-4 border-t border-slate-200 dark:border-slate-700/50">
                <button
                    onClick={toggleTheme}
                ...
                <div className="flex items-center gap-3 px-4 py-3 mb-3 bg-slate-50 dark:bg-slate-900/30 rounded-xl border border-slate-200 dark:border-slate-700/30">
                    <div
                        ...
                        style={{
                            background:
                                'linear-gradient(120deg, var(--zam-sea), var(--zam-coral))'
                        }}
                    >
                    ...
                    <div className="overflow-hidden">
                        ...
                        <p
                            className="text-slate-500 dark:text-slate-400 text-xs truncate"
                            title={agencyUser.email}
                        >
                            {agencyUser.email}
                        </p>
                    </div>
