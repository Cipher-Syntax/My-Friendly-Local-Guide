                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <p className="text-sm text-slate-400">
                                {currentSelectedBooking?.name || currentSelectedBooking?.destination_detail?.name || `Booking #${currentSelectedBooking?.id}`} • <span className="text-white font-semibold">{currentSelectedBooking?.num_guests || currentSelectedBooking?.groupSize || 1} Guests</span>
                            </p>
                            {bookingCategory && (
                                <span className="text-[10px] font-medium px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 flex items-center gap-1">
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="w-12 h-12 rounded-full bg-slate-600 flex items-center justify-center text-lg font-bold text-white shrink-0 uppercase">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-sm flex gap-2 items-center">
                                            <span className={isRecommendedMatch ? "text-cyan-400 font-medium flex items-center gap-1" : "text-slate-400"}>
                                                {guideSpec || 'General'}
                                                {isRecommendedMatch && <Sparkles className="w-3 h-3" title="Matches booking category!" />}
                                            </span>
                                            <span className="text-slate-500">•</span>
                                            <span className={guide.available !== false && guide.is_active !== false ? 'text-green-400' : 'text-red-400'}>
