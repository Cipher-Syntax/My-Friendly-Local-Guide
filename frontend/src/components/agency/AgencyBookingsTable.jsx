        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl overflow-hidden flex flex-col h-full relative">
            {toast.show && (
                <div className={`fixed top-6 right-6 z-[100] px-6 py-4 rounded-lg shadow-2xl border flex items-center gap-3 transition-all duration-300 animate-in fade-in slide-in-from-top-4 ${toast.type === 'success'
                    ? 'bg-slate-800 border-green-500/50 text-green-400' : 'bg-slate-800 border-red-500/50 text-red-400'}`}>
            ...
            <div className="p-4 border-b border-slate-700/50 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-2">
                    ...
                    <span className="bg-slate-700 text-slate-300 text-xs px-2 py-1 rounded-full">{filteredBookings.length}</span>
                </div>
                ...
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-slate-700/30">
                        {filteredBookings.length > 0 ? (
                                    ...
                                    <tr key={booking.id} className="hover:bg-slate-700/20 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-white font-medium truncate max-w-[150px]" title={bookingName}>{bookingName}</p>
                                                    <div className="flex items-center gap-1 mt-0.5">
                                                        <span className="text-[10px] font-medium px-2 py-0.5 rounded bg-slate-700/50 text-cyan-400 border border-cyan-500/20 truncate max-w-[150px] flex items-center gap-1">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-slate-500 text-xs ml-4.5">to {formatDate(booking.check_out || booking.date)}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="text-white text-sm truncate max-w-[120px]" title={booking.location || 'Local'}>{booking.location || 'Local'}</p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="text-white text-sm">{booking.num_guests || booking.groupSize || 1} people</p>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <button
                                                ...
                                                className={`px-3 py-1.5 text-white text-xs font-medium rounded-lg transition-colors whitespace-nowrap ${isManageDisabled ? 'bg-slate-700/50 text-slate-500 cursor-not-allowed' : 'bg-cyan-500 hover:bg-cyan-600 shadow-lg shadow-cyan-500/20'}`}
                                            >
                                                Manage Guides
                                            </button>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <button
                                                    ...
                                                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors w-[80px] ${['accepted', 'paid', 'completed'].includes(booking.status?.toLowerCase()) ? 'bg-green-500 text-white cursor-default' : isDecisionMade ? 'bg-slate-800 text-slate-600 cursor-not-allowed' : 'bg-slate-700/50 text-slate-400 hover:bg-green-500 hover:text-white'}`}
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors w-[80px] ${booking.status?.toLowerCase() === 'declined' ? 'bg-red-500 text-white cursor-default' : isDecisionMade ? 'bg-slate-800 text-slate-600 cursor-not-allowed' : 'bg-slate-700/50 text-slate-400 hover:bg-red-500 hover:text-white'}`}
                                                >
                            ...
                            <tr><td colSpan="8" className="px-6 py-12 text-center text-slate-500"><p className="text-sm">No bookings found</p></td></tr>
                        )}
                    </tbody>
                </table>
            </div>
