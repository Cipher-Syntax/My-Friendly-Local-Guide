...
const StatCard = ({ title, value, subtext, icon: Icon, color }) => (
    <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 relative overflow-hidden group hover:border-slate-600 transition-colors">
...
export default function Dashboard() {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                // Fetching from the new centralized backend endpoint
    ...
    if (error || !data) return (
        <div className="p-8 text-center text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl shadow-lg">
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div>
                ...
                <p className="text-slate-400 mt-1">Live platform analytics and pending actions.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl overflow-hidden flex flex-col shadow-lg">
                    <div className="p-6 border-b border-slate-700/50 flex justify-between items-center">
                        <h3 className="text-white font-semibold flex items-center gap-2">
                            <Calendar className="w-5 h-5 text-cyan-400" />
                            Recent Bookings
                        </h3>
                        <Link to="/admin/bookings" className="text-sm text-cyan-400 hover:text-cyan-300 flex items-center gap-1 transition-colors font-medium">
                                ...
                                {bookings.recent.length > 0 ? (
                                    bookings.recent.map((booking) => (
                                        <tr key={booking.id} className="hover:bg-slate-700/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-white font-medium">{booking.tourist__username}</td>
                                            <td className="px-6 py-4 text-emerald-400 font-medium">₱{parseFloat(booking.total_price || 0).toLocaleString()}</td>
                ...
                <div className="space-y-6">
                    ...
                    <div className={`border rounded-xl p-6 shadow-lg transition-colors ${users.total_pending > 0 ? 'bg-orange-500/5 border-orange-500/20' : 'bg-slate-800/50 border-slate-700/50'}`}>
                        <h3 className="text-white font-semibold flex items-center gap-2 mb-4">
                            <AlertTriangle className={`w-5 h-5 ${users.total_pending > 0 ? 'text-orange-400 animate-pulse' : 'text-slate-400'}`} />
                            Needs Attention
                        </h3>

                        <div className="space-y-3">
                            <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg border border-slate-700 hover:border-slate-500 transition-colors">
                                <div className="flex items-center gap-3">
                                    <div className="p-2 bg-blue-500/10 rounded-lg">
                                        <UserCheck className="w-5 h-5 text-blue-400" />
                                    </div>
                                    <div>
                                        <h4 className="text-white text-sm font-medium">Guides</h4>
                                        <p className="text-slate-400 text-xs">{users.pending_guides} pending</p>
                                    </div>
                                </div>
                                <Link to="/admin/guides" className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-medium rounded-lg transition-colors border border-slate-600 shadow-sm">
                                    Review
                                </Link>
                            </div>
                            ...
                            <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg border border-slate-700 hover:border-slate-500 transition-colors">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4 className="text-white text-sm font-medium">Agencies</h4>
                                        <p className="text-slate-400 text-xs">{users.pending_agencies} pending</p>
                                    </div>
                                </div>
                                <Link to="/admin/agency" className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-medium rounded-lg transition-colors border border-slate-600 shadow-sm">
                                    Review
                                </Link>
                            </div>
                        </div>
                        ...
                        <div className="space-y-5">
                            <div>
                                <div className="flex justify-between mb-2 text-sm">
                                    <span className="text-slate-400 font-medium">Tourists</span>
                                    <span className="text-white font-bold">{users.tourists}</span>
                                </div>
                                <div className="w-full bg-slate-900/80 rounded-full h-2.5 shadow-inner">
                            ...
                            <div>
                                <div className="flex justify-between mb-2 text-sm">
                                    <span className="text-slate-400 font-medium">Tour Guides</span>
                                    <span className="text-white font-bold">{users.guides}</span>
                                </div>
                                <div className="w-full bg-slate-900/80 rounded-full h-2.5 shadow-inner">
                            ...
                            <div>
                                <div className="flex justify-between mb-2 text-sm">
                                    <span className="text-slate-400 font-medium">Agencies</span>
                                    <span className="text-white font-bold">{users.agencies}</span>
                                </div>
                                <div className="w-full bg-slate-900/80 rounded-full h-2.5 shadow-inner">
