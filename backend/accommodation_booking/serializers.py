...
class BookingSerializer(serializers.ModelSerializer):
    ...
    # This field is calculated by the method below
    assigned_guides_detail = serializers.SerializerMethodField(read_only=True)
    ...
    def get_guide_detail(self, obj):
        if obj.guide:
            from user_authentication.serializers import UserSerializer
            # Passing context allows UserSerializer to build absolute URLs for images too
    ...
    def get_assigned_guides_detail(self, obj):
        """
        Returns a lightweight list of assigned guides specifically formatted 
        for easy parsing in the frontend Payment screen.
        Now generates ABSOLUTE URLs for profile pictures.
        """
        ...
        for guide in obj.assigned_guides.all():
            ...
            if guide.profile_picture:
                ...
                # This line converts '/media/...' to 'http://.../media/...'
                if request:
